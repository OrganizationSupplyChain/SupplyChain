            ...
            try
            {
                ...
                if (authorized)
                {
                    ...
                    if (authStatus == "COMPLETED")
                    {
                        ...
                        var loanUpdated = _dbcalls.UpdateInvoiceLoan(model.id, disbursed, model.days);
                        if (!disbursed)
                        {
                            auditMsg = "Invoice successfully approved, awaiting disbursement";
                        }
                    }
                    else
                    {
                    ...
                    SendMailReq mailReq = new SendMailReq()
                    {
                        Content = $"<p>Good day {model.vendorname},</p><p>Your Invoice request on Access Bank Supply Chain platform has been {authStat} by {model.AUTHORIZERNAME}.  <p> Login <a href={supplyChainUrl}>here</a> to the portal to view details. </p><p> Regards, Supply Chain Admin</p>",
                        CopyAddress = "",
                        From = "no-reply@accessbankplc.com",
